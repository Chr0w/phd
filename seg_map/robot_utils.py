...
SECTION_CULL_UPDATE_INTERVAL_S = 0.5


def distance_to_point(robot_x: float, robot_y: float, x: float, y: float) -> float:
    return math.hypot(float(x) - float(robot_x), float(y) - float(robot_y))


def item_should_be_visible(distance_m: float, currently_visible: bool) -> bool:
